...
export default function ProgressWeeklyScreen({
  ...
  const stats = useMemo(() => {
    ...
    for (let i = 0; i <= dayOfWeek; i++) {
      ...
      const metGoal =
        dailyTotals[i] > 0 &&
        dailyTotals[i] <= goals.calories + 100 &&
        dailyTotals[i] >= goals.calories - 400;
      if (metGoal) {
        goalsMet++;
        currentStreak++;
      } else {
    ...
    const avgCalories =
      daysWithFood > 0 ? Math.round(sumCalories / daysWithFood) : 0;
    const dayNamesFull = [
      'Monday',
      'Tuesday',
      'Wednesday',
  ...
  return (
    <ScreenShell
      ...
      onBack={onBack}
      footer={
        <PrimaryButton onClick={() => onNavigate('progress-monthly')}>
      ...
      <div className="space-y-5 pb-8">
          ...
          <div className="flex h-36 items-end justify-between px-2 pt-2">
            {stats.trend.map((kcal, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex flex-col items-center gap-2 w-8 text-center"
                >
                  ...
                  <div className="w-full bg-[#f2faf5] rounded-full h-24 relative overflow-hidden">
                    <div
                      ...
                      style={{
                        height: `${percent}%`,
                        background: isOverGoal ? '#fb923c' : green,
                      }}
                    />
                  </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <span
              className="text-lg font-black block mt-1.5 text-foreground"
          ...
          >
            ...
            <span className="text-lg font-black block mt-1.5 text-[#34C759]">
              {stats.healthiestDay}
            </span>
          </div>
          <div
          ...
          >
            ...
            <span className="text-lg font-black block mt-1.5 text-[#0EA5E9]">
              {stats.proteinMet} / 7 Days
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p className="text-xs font-bold text-[#197a38] uppercase tracking-wide">
              Liva Weekly Summary
            </p>
            <p
