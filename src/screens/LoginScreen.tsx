...
export default function LoginScreen({
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isPlaceholderConfig() && email && password) {
        console.warn(
          'Firebase sign in failed, checking offline mock credentials fallback:',
          err
        );
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isPlaceholderConfig() && email && password) {
        console.warn(
          'Firebase sign up failed, falling back to local storage profile creation:',
          err
        );
        const mockUid = 'mock_' + email.replace(/[@.]/g, '_');
        const profileData = {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(
        'A password reset email has been dispatched via Firebase Auth.'
      );
    } catch (err: any) {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    setSuccessMsg(
      'Please use the link sent to your inbox to reset your password.'
    );
    changeView('signin');
  };
  ...
  const handleGoogleOAuth = async () => {
    ...
    try {
      ...
      if (isPlaceholderConfig()) {
        console.warn(
          'Firebase Google login failed, checking fallback simulator:',
          err
        );
        // Fallback
  ...
  const handleSaveProfileCompletion = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await withFirebaseTimeout(
        update(ref(db, `users/${activeSessionToken!}`), updates),
        'Database update'
      );

      const userSnapshot = await withFirebaseTimeout(
        get(ref(db, `users/${activeSessionToken!}`)),
        'Database read'
      );
      onAuthSuccess(activeSessionToken!, userSnapshot.val(), true);
    } catch (err: any) {
      console.warn(
        'Firebase profile save failed, writing to localStorage fallback:',
        err
      );
      const dobCombined = `${dobYear}-${dobMonth}-${dobDay}`;
  ...
  return (
    <ScreenShell footer={<ProgressDots total={5} current={1} />}>
      <div className="flex h-full flex-col justify-center min-h-0 overflow-y-auto pb-4 px-3 relative">
        ...
        {view !== 'profile-complete' && (
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground" >
              ...
              {view === 'forgot' &&
                'Input your registered email to request a reset link.'}
              {view === 'reset' &&
                'Provide the token and set a strong password.'}
              {view === 'verify' &&
                'A validation link was printed to server console logs.'}
            </p>
          </motion.div>
        )}
          ...
          <form onSubmit={handleSignUp} className="flex flex-col gap-2.5">
            ...
            <div className="flex items-start gap-2 py-1">
              ...
              >
                ...
                and{' '}
                <span className="text-[#34c759] underline">Privacy Policy</span>
                .
              </label>
            </div>
        ...
        {view === 'profile-complete' && (
          <form
            onSubmit={handleSaveProfileCompletion}
