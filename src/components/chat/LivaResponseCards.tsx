...
export const FoodRecommendationCard: React.FC<FoodRecommendationCardProps> = ({
  ...
  return (
    <div className="flex flex-col gap-2 w-full animate-fadeIn mt-1">
      {meal && (
        <div className="bg-[#f8faf8] border border-slate-100 dark:border-border rounded-xl p-3 shadow-sm flex justify-between items-center">
          <span className="font-bold text-[14px] text-foreground">{meal}</span>
          {calories && (
      ...
      {why && why.length > 0 && (
        <div className="pl-1">
          <h4 className="text-[12px] font-bold text-foreground mb-1">
            Why it's good:
          </h4>
          <ul className="list-disc pl-4 space-y-0.5">
            {why.map((reason, idx) => (
              <li key={idx} className="text-[12px] text-muted-foreground leading-snug">
                {reason}
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="w-full animate-fadeIn mt-1">
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <h4 className="text-[13px] font-bold text-foreground">Alternatives</h4>
      </div>

      <div className="flex flex-col rounded-xl border border-slate-100 dark:border-border bg-card text-card-foreground overflow-hidden shadow-sm">
        {alternatives.map((alt, idx) => {
          ...
          return (
            <div key={idx}>
              {idx > 0 && (
                <div className="border-t border-dashed border-slate-100 dark:border-border mx-3" />
              )}
              <div className="flex items-center justify-between px-3 py-2 group hover:bg-slate-50 dark:hover:bg-muted transition-colors cursor-pointer">
...
export const NutritionSummaryCard = ({
  ...
  return (
    <div className="w-full mt-2 bg-card text-card-foreground rounded-xl border border-slate-100 dark:border-border shadow-sm p-3 animate-fadeIn">
      <div className="grid grid-cols-4 gap-1">
        <div className="flex flex-col items-center justify-center border-r border-slate-100 dark:border-border">
          <span className="text-[10px] font-semibold text-muted-foreground mb-0.5">
            Cals
          </span>
          <div className="font-extrabold text-foreground text-[13px] leading-none">
            {calories}
          </div>
        </div>
        <div className="flex flex-col items-center justify-center border-r border-slate-100 dark:border-border">
