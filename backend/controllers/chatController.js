...
- CRITICAL: If logging a meal, set action.type to "MEAL_LOG" and set action.data.mealType to one of: "breakfast", "lunch", "dinner", "snack". If the user did NOT explicitly mention which meal they ate (e.g., they just said "I had 2 rotis" without specifying "for breakfast"), you MUST set mealType to "unknown". NEVER guess the meal type based on time. Always use "unknown" so the app can ask the user with a cue card.
- CRITICAL: When the user logs multiple items at once (e.g. "dal rice and prawns fry"), you MUST separate them into individual elements in the "action.data.items" array (e.g. ["dal rice", "prawns fry"]). Calculate the total combined calories and macros for all items.
- CRITICAL: When extracting food names for "action.data.items", STRICTLY remove any conversational filler or meal indicators like "for breakfast", "in snacks", "for lunch", "i had", "and", etc. The items should be the pure food name and quantity ONLY (e.g. ["dal rice", "prawns fry"], not ["i had dal rice", "and prawns fry in lunch"]).
...
- CRITICAL NUTRITION CONSISTENCY RULE: If the user logs a food item that exactly or closely matches an item they have logged previously (see "User's Recent Logged Meals"), you MUST reuse the EXACT same calories, protein, carbs, and fat you assigned to it last time. NEVER generate different nutrition values for the same food. Maintain extreme accuracy for the user's trust.
...
`;
    ...
    // Fail-safe: Enforce historical calorie consistency if AI hallucinated
    if (mealData && mealData.items && mealData.items.length > 0) {
      const currentItemsStr = mealData.items.join(', ').toLowerCase();
      // Try to find a previous meal with the exact same items string
      const exactMatch = combinedMeals.find(
        (m) => m.foodItem && m.foodItem.toLowerCase() === currentItemsStr
      );

      if (exactMatch) {
        // Overwrite the AI's hallucinated calories with the consistent historical ones
        mealData.calories = exactMatch.calories;
        mealData.protein = exactMatch.protein;
        mealData.carbs = exactMatch.carbs;
        mealData.fat = exactMatch.fat;
      }
    }
